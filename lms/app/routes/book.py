from flask import current_app, Blueprint, g, escape, session, redirect, render_template, request, jsonify, Response
from app import DAO
...
def home(id):
  ...
  if id != None:
    ...
    if user_manager.user.isLoggedIn():
      user_books = book_manager.getReserverdBooksByUser(user_id=user_manager.user.uid())
      if user_books is not None:
        if "user_books" in user_books:
          user_books = user_books['user_books'].split(',')
        else:
          user_books = []
      else:
        user_books = []
